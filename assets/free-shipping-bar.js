...
setInterval(fsbInstallFetchPatch, 2000);
  ...
  function fsbRunCycle() {
    ...
    return fetchCart()
      .then(function(cart) {
        return fsbReconcileGifts(cart, config.giftTiers, config.giftTitles).then(function(verifiedCart) {
          var qualifying = fsbQualifyingTotal(verifiedCart);
          return fsbApplyDiscountCodes(verifiedCart, qualifying, config.discountTiers);
        });
      })
      .then(function() {
